...
public function get_appointment($venue_id=NULL,$date=NULL)

{
    ...
    #---------------------------------------------    
     ...
     if($result){
      echo'<div style="font-size:15px; line-height:24px; color:#E15800;">' . $result->message. '</div>';
        ...
        $re = $this->chackSchedulDate_doctor($venue_id,$date);    

        if($re==TRUE){

            $result1 = $this->chackSchedulDate($venue_id,$date);
                        ...
                        for ($i = 1; $i <= $per_patient_time; $i++) {
                         ...
                         // $patient_time = date('h:i A', strtotime(strtotime($start_time)+$time*60));
                          $patient_time =date('h:i A', strtotime($result1->start_time)+$time*60);
                           ...
                           $button_color = $this->Appointment_checker($venue_id,$date,$patient_time);
  ...
  # Appointment_checker
  #----------------------------------------
  public function Appointment_checker($venue_id,$date,$sequence) 
...
#---------------------------------------------    
  ...
  public function chackEmergencyStop($date)
  {
   ...
   public function chackSchedulDate_doctor($venue_id,$date)
  ...
  // Chack date doctor
  public function chackSchedulDate($venue_id,$date)
  {   
